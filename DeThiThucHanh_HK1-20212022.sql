...
--	Cách 1: Sử dụng CONSTRAINT
ALTER TABLE NHANVIEN
ADD CONSTRAINT Check_LuongNhanVien_PhongNC
CHECK (MAPHG != 'NC' OR (MAPHG = 'NC' AND LUONG > 20000000))

--	Cách 2: Sử dụng TRIGGER
GO
CREATE TRIGGER Trigger_InsertUpdate_LuongNhanVien_PhongNC
ON NHANVIEN
FOR INSERT, UPDATE
AS
BEGIN
	...
	SELECT @MAPHG = MAPHG, @LUONG = LUONG
	FROM inserted
	
	IF (@MAPHG = 'NC' AND @LUONG <= 20000000)
	BEGIN
		PRINT 'ERROR: NHANVIEN MAPHG=NC MUST HAVE LUONG > 20.000.000'
		ROLLBACK TRANSACTION
	END
	ELSE
		PRINT 'INSERT/UPDATE DATA SUCCESSFULLY!'
END
...
--	Table: PHANCONG
GO
CREATE TRIGGER Trigger_InsertUpdate_PHANCONG
ON PHANCONG
FOR INSERT, UPDATE
AS
BEGIN
	...
	SELECT @NGBD_DK = DEAN.NGBD_DK, @NGSINH = NHANVIEN.NGSINH
	FROM PHANCONG, DEAN, NHANVIEN
	WHERE PHANCONG.MANV = NHANVIEN.MANV AND PHANCONG.MADA = DEAN.MADA

	IF (@NGBD_DK <= @NGSINH)
	BEGIN
		PRINT 'ERROR: NGBD_DK MUST BE BIGGER THAN NGSINH (NGBD_DK > NGSINH)'
		ROLLBACK TRANSACTION
	END
	ELSE
		PRINT 'INSERT/UPDATE DATA SUCCESSFULLY'
...
BEGIN
	IF EXISTS (SELECT *
				FROM PHANCONG, inserted, DEAN
				WHERE PHANCONG.MANV = inserted.MANV AND PHANCONG.MADA = DEAN.MADA
					AND DEAN.NGBD_DK <= inserted.NGSINH)
	BEGIN
		PRINT 'ERROR: NGBD_DK MUST BE BIGGER THAN NGSINH (NGBD_DK > NGSINH)'
		ROLLBACK TRANSACTION
	END
...
--	Table: DEAN
GO
CREATE TRIGGER Trigger_Update_DEAN
ON DEAN
FOR UPDATE
AS
BEGIN
	IF EXISTS (SELECT *
				FROM PHANCONG, inserted, NHANVIEN
				WHERE PHANCONG.MADA = inserted.MADA AND PHANCONG.MANV = NHANVIEN.MANV
					AND inserted.NGBD_DK <= NHANVIEN.NGSINH)
	BEGIN
		PRINT 'ERROR: NGBD_DK MUST BE BIGGER THAN NGSINH (NGBD_DK > NGSINH)'
		ROLLBACK TRANSACTION
	END
